...
M = 10000000  # or another reasonable upper bound for BESS power
model.soc_limit = pyo.ConstraintList()

for t in T:
    model.soc_limit.add(model.charge[t] <= M * model.is_charging[t])
    model.soc_limit.add(model.discharge[t] <= M * model.is_discharging[t])
    model.soc_limit.add(model.is_charging[t] + model.is_discharging[t] <= 1)
